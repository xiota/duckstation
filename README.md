# LibreStation - PlayStation Emulator

LibreStation is an simulator/emulator of the Sony PlayStation(TM) console, focusing on playability, speed, and long-term maintainability. The goal is to be as accurate as possible while maintaining performance suitable for low-end devices. "Hack" options are discouraged, the default configuration should support all playable games with only some of the enhancements having compatibility issues.
...
LibreStation features a fully-featured frontend built using Qt, as well as a fullscreen/TV UI based on Dear ImGui.
...
## System Requirements
 - A CPU faster than a potato. But it needs to be x86_64 (SSE4.1), AArch32/armv7, AArch64/ARMv8, or RISC-V/RV64.
   - If you want to use LibreStation with an older x86_64 CPU that does not support SSE4.1, [v0.1-6995](tree/38774867121728f19cc6dd48104e394bed911a30) is the last version that does not require SSE4.1.
 - For the hardware renderers, a GPU capable of OpenGL 3.1/OpenGL ES 3.1/Direct3D 11 Feature Level 10.0 (or Vulkan 1.0) and above. So, basically anything made in the last 10 years or so.
 - SDL, XInput or DInput compatible game controller (e.g. XB360/XBOne/XBSeries). DualShock 3 users on Windows will need to install the official DualShock 3 drivers included as part of PlayStation Now.

### Windows

LibreStation **requires** Windows 10/11, specifically version 1809 or newer. If you are still using Windows 7/8/8.1, LibreStation **will not run** on your operating system. Running these operating systems in 2023 should be considered a security risk, and I would recommend updating to something which receives vendor support.
If you must use an older operating system, [v0.1-5624](tree/39e62ae948d7b123ec9a60e7cea71243b030e85a) is the last version which will run. But do not expect to recieve any assistance, these builds are no longer supported.

**If you get an error about `vcruntime140_1.dll` being missing, you need to update your Visual C++ runtime.** You can download the installer from [Microsoft Visual C++ Redistributable downloads](https://support.microsoft.com/en-au/help/2977003/the-latest-supported-visual-c-downloads).
Specifically, you need the x64 runtime, which can be downloaded from [`vc_redist.x64.exe`](https://aka.ms/vs/17/release/vc_redist.x64.exe).
...
For these games, make sure that the CD image and its corresponding SBI (.sbi) file have the same name and are placed in the same directory. LibreStation will automatically load the SBI file when it is found next to the CD image.
...
2. Download the [dependencies pack](https://github.com/stenzek/duckstation-ext-qt-minimal/releases/) and extract it to `dep\msvc`.
3. Open the Visual Studio solution `librestation.sln` in the root, or "Open Folder" for cmake build.
4. Build solution.
5. Binaries are located in `bin/x64`.
6. Run the executable corresponding to the config you used.
...
5. Run the binary, located in the build directory under `./build-release/bin/librestation-qt`.
...
5. Run the binary, located in the build directory under `bin/LibreStation.app`.
...
- Windows: My Documents\LibreStation
- Linux: `$XDG_DATA_HOME/librestation`, or `~/.local/share/librestation`.
- macOS: `~/Library/Application Support/LibreStation`.

So, if you were using Linux, you would place your BIOS images in `~/.local/share/librestation/bios`. This directory will be created upon running LibreStation
for the first time.
...
in the same directory as the LibreStation executable.
...
LibreStation releases ship with a database of game controller mappings for the SDL controller backend, courtesy of https://github.com/mdqinc/SDL_GameControllerDB. The included `gamecontrollerdb.txt` file can be found in the `resources` subdirectory of the LibreStation program directory.
