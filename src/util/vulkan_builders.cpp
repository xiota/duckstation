...
void Vulkan::DescriptorSetLayoutBuilder::AddBinding(u32 binding, VkDescriptorType dtype, u32 dcount,
                                                    VkShaderStageFlags stages)
{
  DebugAssert(m_ci.bindingCount < MAX_BINDINGS);
...
{
  DebugAssert(m_ci.setLayoutCount < MAX_SETS);
...
void Vulkan::GraphicsPipelineBuilder::SetShaderStage(VkShaderStageFlagBits stage, VkShaderModule module,
                                                     const char* entry_point)
{
  DebugAssert(m_ci.stageCount < MAX_SHADER_STAGES);
...
void Vulkan::GraphicsPipelineBuilder::AddVertexBuffer(u32 binding, u32 stride,
...
void Vulkan::GraphicsPipelineBuilder::SetPrimitiveTopology(VkPrimitiveTopology topology,
...
void Vulkan::GraphicsPipelineBuilder::SetRasterizationState(VkPolygonMode polygon_mode, VkCullModeFlags cull_mode,
...
void Vulkan::GraphicsPipelineBuilder::SetStencilState(bool stencil_test, const VkStencilOpState& front,
...
void Vulkan::GraphicsPipelineBuilder::AddBlendAttachment(
...
{
  DebugAssert(m_blend_state.attachmentCount < MAX_ATTACHMENTS);
...
{
  DebugAssert(m_dynamic_state.dynamicStateCount < MAX_DYNAMIC_STATE);
...
void Vulkan::GraphicsPipelineBuilder::SetViewport(float x, float y, float width, float height, float min_depth,
                                                  float max_depth)
{
  m_viewport.x = x;
  m_viewport.y = y;
...
void Vulkan::DescriptorSetUpdateBuilder::PushUpdate(VkCommandBuffer cmdbuf, VkPipelineBindPoint bind_point,
                                                    VkPipelineLayout layout, u32 set, bool clear /*= true*/)
{
  DebugAssert(m_num_writes > 0);
...
void Vulkan::DescriptorSetUpdateBuilder::AddImageDescriptorWrite(
  VkDescriptorSet set, u32 binding, VkImageView view,
  VkImageLayout layout /*= VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL*/)
{
  DebugAssert(m_num_writes < MAX_WRITES && m_num_image_infos < MAX_IMAGE_INFOS);
...
void Vulkan::DescriptorSetUpdateBuilder::AddSamplerDescriptorWrites(VkDescriptorSet set, u32 binding,
                                                                    const VkSampler* samplers, u32 num_samplers)
{
  DebugAssert(m_num_writes < MAX_WRITES && (m_num_image_infos + num_samplers) < MAX_IMAGE_INFOS);
...
void Vulkan::DescriptorSetUpdateBuilder::AddCombinedImageSamplerDescriptorWrite(
...
void Vulkan::DescriptorSetUpdateBuilder::AddCombinedImageSamplerDescriptorWrites(
  VkDescriptorSet set, u32 binding, const VkImageView* views, const VkSampler* samplers, u32 num_views,
  VkImageLayout layout /* = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL */)
{
  DebugAssert(m_num_writes < MAX_WRITES && (m_num_image_infos + num_views) < MAX_IMAGE_INFOS);
...
void Vulkan::DescriptorSetUpdateBuilder::AddBufferDescriptorWrite(VkDescriptorSet set, u32 binding,
                                                                  VkDescriptorType dtype, VkBuffer buffer, u32 offset,
                                                                  u32 size)
{
  DebugAssert(m_num_writes < MAX_WRITES && m_num_buffer_infos < MAX_BUFFER_INFOS);
...
void Vulkan::DescriptorSetUpdateBuilder::AddBufferViewDescriptorWrite(VkDescriptorSet set, u32 binding,
                                                                      VkDescriptorType dtype, VkBufferView view)
{
  DebugAssert(m_num_writes < MAX_WRITES && m_num_views < MAX_VIEWS);
...
{
  DebugAssert(m_ci.attachmentCount < MAX_ATTACHMENTS);
...
{
  DebugAssert(m_ci.subpassCount < MAX_SUBPASSES);
...
{
  DebugAssert(subpass < m_ci.subpassCount && m_num_attachment_references < MAX_ATTACHMENT_REFERENCES);
