...
{
  return QStringLiteral("LibreStation %1").arg(QLatin1StringView(g_scm_tag_str));
}
...
bool QtHost::InitializeConfig(std::string settings_filename)
{
      ...
      settings_version != SETTINGS_VERSION)
  {
    ...
    {
      QMessageBox::critical(
        nullptr, QStringLiteral("LibreStation"),
        QStringLiteral(
          "Failed to save configuration to\n\n%1\n\nThe error was: %2\n\nPlease ensure this directory is writable. You "
          "can also try portable mode by creating portable.txt in the same directory you installed LibreStation into.")
...
bool QtHost::SetCriticalFolders()
{
  ...
  const std::string rcc_path = Path::Combine(EmuFolders::Resources, "librestation-qt.rcc");
...
#if defined(_WIN32)
  // On Windows, use My Documents\LibreStation.
  PWSTR documents_directory;
  if (SUCCEEDED(SHGetKnownFolderPath(FOLDERID_Documents, 0, NULL, &documents_directory)))
  {
    if (std::wcslen(documents_directory) > 0)
      EmuFolders::DataRoot = Path::Combine(StringUtil::WideStringToUTF8String(documents_directory), "LibreStation");
    CoTaskMemFree(documents_directory);
  }
#elif defined(__linux__) || defined(__FreeBSD__)
  // Use $XDG_CONFIG_HOME/librestation if it exists.
  const char* xdg_config_home = getenv("XDG_CONFIG_HOME");
  if (xdg_config_home && Path::IsAbsolute(xdg_config_home))
  {
    EmuFolders::DataRoot = Path::RealPath(Path::Combine(xdg_config_home, "librestation"));
  }
  else
  {
    // Use ~/.local/share/librestation otherwise.
    ...
    if (home_dir)
    {
      ...
      EmuFolders::DataRoot = Path::RealPath(Path::Combine(share_dir, "librestation"));
    }
  }
#elif defined(__APPLE__)
  static constexpr char MAC_DATA_DIR[] = "Library/Application Support/LibreStation";
  const char* home_dir = getenv("HOME");
  if (home_dir)
    ...
    if (!FileSystem::EnsureDirectoryExists(EmuFolders::DataRoot.c_str(), false, &error))
    {
      ...
      QMessageBox::critical(
        ...
        QStringLiteral("Failed to create data directory at path\n\n%1\n\nThe error was: %2\nPlease ensure this "
                       ...
                       "directory you installed LibreStation into.")
...
void QtHost::PrintCommandLineVersion()
{
  ...
  std::fprintf(stderr, "LibreStation Version %s (%s)\n", g_scm_tag_str, g_scm_branch_str);
  std::fprintf(stderr, "\n");
}
