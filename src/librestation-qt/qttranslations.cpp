...
#define MAKE_FONT_DOWNLOAD_URL(name) "https://www.librestation.org/runtime-resources/fonts/" name
...
void QtHost::UpdateApplicationLanguage(QWidget* dialog_parent)
{
  ...
  const QString path = QStringLiteral("%1/librestation-qt_%3.qm").arg(base_dir).arg(language);
...
static constexpr const QtHost::GlyphInfo s_glyph_info[] = {
...

  {
    ...
    // auto update by generate_update_glyph_ranges.py with librestation-qt_ja.ts
  ...
  },
  {
    ...
    // auto update by generate_update_glyph_ranges.py with librestation-qt_ko.ts
  ...
  },
  {
    ...
    // auto update by generate_update_glyph_ranges.py with librestation-qt_zh-cn.ts
